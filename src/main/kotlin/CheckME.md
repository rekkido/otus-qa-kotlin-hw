...
* Резюме:
* Задание принимается, запускается, срабатывает правильно
  Для котлина иметь !! в окончательном тексте очень нехорошо - посмотрите как решен вопрос в Example
* Настоятельно рекомендую для этого задания выполнить шаги, указанные внизу, и доделать пример до DSL
...
Оценка:
1. Не реализован поиск after и before в классе, да и тестового класса с несколькими after и before нет.
   interface TestSteps неверна реализация  - не соответствует заданию
